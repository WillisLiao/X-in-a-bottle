...
**Latest, sixth session - recoil, muzzle flash, ballistic shield rework, first-person hands (via `/sonnet-opus`):**
Recoil now moves the weapon itself (a linear throw on `_weapon_rig.position`, not just the old rotation-only kick), and rides through to ADS since the sight is a child of the same rig. Muzzle flash is a real crossed-blade flash plus a brief point light on all six weapon builds, not a glowing box. First-person hands exist for the first time - a tactical glove, `frontend-opus`-designed after my first pass was rejected as too small and too close to the placeholder body's look; team-neutral by explicit instruction (Red/Blue is a match label, never a hand color). The Shield class got a real rework: default carry recentred in front, melee bumps the shield forward instead of swinging the weapon, reload tucks the weapon behind the vertical shield, a new revolver model (Shield class only - Runner keeps the semi-auto), and an ADS pose that pitches the plate into a low horizontal parapet with the sidearm rising to a normal centered sight picture - corrected mid-session from an initial (wrong) screen-plane-roll interpretation, using reference screenshots the user supplied from an actual shield-operator FPS. Vision port enlarged to near-full plate width and 3x its original height, and a rib that would have crossed the enlarged opening was removed. Deployed to the iPad Pro at session end. Full detail, including a methodology note on a `game_eval`/`await`-loop testing trap that cost real time this session: "Sixth session" in `devlogs/2026-08-07.md`.
...
- `handoffs/NEXT-SESSION-weapon-visual-bugs.md` - **highest priority of the three**, filed straight from the deployed iPad build. Six real bugs the user found by actually playing: ADS reticle doesn't move with recoil, recoil/muzzle-flash intermittently stop working after respawn or a class switch, the sniper shows no visible recoil at all while scoped, the sniper's reticle should be a dot not a cross, the pistol should be one-handed at hip and two-handed only on ADS (and the support hand currently comes from the wrong side entirely), and the shield's default carry still isn't "directly in front" plus its ADS hand/shield/tilt composition is wrong. File has file:line pointers, which parts are confirmed root causes vs. still-open, and a testing-methodology trap (a `game_eval`/`await`-loop pattern that reliably produces false "doesn't work" readings) worth reading before repeating the mistake.
