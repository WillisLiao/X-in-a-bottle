...
func _draw() -> void:
	...
	draw_string(_font, Vector2(52.0, 31.0), "RIFTLINE / FIELD DRILLS", HORIZONTAL_ALIGNMENT_LEFT, -1.0, 13, MUTED)
	if _board:
		_draw_board()
	else:
		_draw_entry()
...
func _draw_entry() -> void:
	var left := 70.0
	var top := maxf(110.0, size.y * 0.18)
	draw_string(_font, Vector2(left, top), "STEP INTO THE STORM", HORIZONTAL_ALIGNMENT_LEFT, -1.0, 34, PAPER)
	draw_string(_font, Vector2(left, top + 34.0), "A quiet relay bay. A live Seed. A crew that moves with purpose.", HORIZONTAL_ALIGNMENT_LEFT, minf(size.x - 120.0, 720.0), 15, MUTED)
	var diagram_center := Vector2(size.x * 0.68, top + 28.0)
	...
	draw_string(_font, Vector2(left, top + 145.0), _drill_description(_selected), HORIZONTAL_ALIGNMENT_LEFT, minf(size.x - 140.0, 620.0), 15, PAPER)
	var action_y := size.y - 148.0
	...
	draw_string(_font, Vector2(left, size.y - 48.0), "OFFLINE PRACTICE / LAST DRILL REMEMBERED", HORIZONTAL_ALIGNMENT_LEFT, -1.0, 12, MUTED)

func _draw_board() -> void:
	var left := 70.0
	var top := maxf(92.0, size.y * 0.13)
	...
	var card_width := minf(340.0, (size.x - 180.0) / 3.0)
	var gap := 28.0
	...
	_register_button("back", Rect2(size.x - 210.0, size.y - 100.0, 140.0, 52.0))
